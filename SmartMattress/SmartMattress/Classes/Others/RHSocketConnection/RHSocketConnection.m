...
+ (instancetype)ShareRHSocketConnection{
    static id iSocketCilent = nil;
//    if (iSocketCilent) {
//        return iSocketCilent;
//    }
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        iSocketCilent = [[self alloc] init];
    });
    
    NSString* _serverHost = serverHost;
    int _serverPort = serverPort;
    [iSocketCilent connectWithHost:_serverHost port:_serverPort];
    return iSocketCilent;
}
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
    ...
    //RHSocketLog(@"[RHSocketConnection] didReadData data: %@, length: %lu, tag: %ld", dataStr, (unsigned long)data.length, tag);
...
- (void)socket:(GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag {
    //RHSocketLog(@"[RHSocketConnection] didWriteDataWithTag is: %ld", tag);
    [sock readDataWithTimeout:-1 tag:tag];
}
